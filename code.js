...
const isSimilarHue = (colorHue, themeHue) => {
    if (colorHue === themeHue)
        return true;
    return ((colorHue > themeHue && colorHue - themeHue <= 5) ||
...
const isSimilarSaturation = (colorSaturation, themeSaturation) => {
    if (colorSaturation === themeSaturation)
        return true;
    return ((colorSaturation > themeSaturation &&
...
const isSimilarLight = (colorLight, themeLight) => {
    if (colorLight === themeLight)
        return true;
    return ((colorLight > themeLight && colorLight - themeLight <= 5) ||
...
const isSimilarColor = (color, themeColor, colorOpacity, themeOpacity) => {
    ...
    return (isSameOpacity(colorOpacity, themeOpacity) &&
...
const isSameStyles = (colors, styles) => {
    if (colors.length !== styles.length)
        return;
    if (colors[0].type !== styles[0].type)
        return;
    let flag = colors.length;
    for (let index = 0; index < colors.length; index++) {
        if (colors[index].visible === false)
            return;
        if (colors[index].color !== undefined &&
            styles[index].color !== undefined) {
            if (colors[index].type === 'SOLID' &&
                isSimilarColor(colors[index].color, styles[index].color, colors[index].opacity, styles[index].opacity)) {
                flag--;
            }
        }
        else if (colors[index].type === 'IMAGE' &&
...
const fixColor = (node, themeStyles) => {
    if (node.fillStyleId === themeStyles.id ||
...
const isInstanceOrParentInstance = (obj) => {
    if (obj.type === 'INSTANCE' || obj.parent.type === 'INSTANCE')
        return true;
...
const main = () => {
    ...
    const allObjectsOnPage = figma.currentPage.findAll((item) => item.type === 'LINE' ||
        item.type === 'ELLIPSE' ||
        item.type === 'POLYGON' ||
        item.type === 'VECTOR' ||
        item.type === 'STAR' ||
